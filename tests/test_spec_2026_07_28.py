...
def _modern_request(
    ...
    *,
    ...
    request_params["_meta"] = {
        ...
        CLIENT_INFO_META_KEY: {"name": "cosmolex-spec-test", "version": "0"},
    }
    headers = {
...
async def _post_modern(
    ...
    *,
    ...
    omitted_headers: set[str] | None = None,
) -> httpx.Response:
    ...
    for header in omitted_headers or ():
...
def test_modern_discovery_is_sessionless_and_declares_only_used_capabilities() -> None:
    ...
    assert result["_meta"][SERVER_INFO_META_KEY]["name"] == "cosmolex"


def test_client_negotiates_modern_by_default_and_keeps_legacy_compatibility() -> None:
...
def test_cacheable_results_are_complete_private_and_deterministic() -> None:
    ...
    assert [item["uri"] for item in resources["resources"]] == [
        "cosmolex://users",
        "cosmolex://clients",
        "cosmolex://security-notes",
    ]
    assert templates["resourceTemplates"] == []

    for tool in first_tools["tools"]:
...
def test_resource_read_cache_hints_and_not_found_error() -> None:
    found = asyncio.run(
        _post_modern("resources/read", {"uri": "cosmolex://security-notes"})
    )
    result = _result(found)
    assert result["resultType"] == "complete"
    assert result["ttlMs"] == 0
    assert result["cacheScope"] == "private"
    assert "attorney-client" in result["contents"][0]["text"]

    missing = asyncio.run(
        _post_modern("resources/read", {"uri": "cosmolex://does-not-exist"})
    )
    assert missing.status_code == 400
    assert missing.json()["error"]["code"] == -32602


def test_tool_schema_rejects_out_of_range_page_size_as_complete_error() -> None:
...
def test_modern_http_enforces_routing_headers_versions_and_method_errors() -> None:
    ...
    missing_method = asyncio.run(
        _post_modern("tools/list", omitted_headers={"mcp-method"})
    )

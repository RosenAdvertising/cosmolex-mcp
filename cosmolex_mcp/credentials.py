#!/usr/bin/env python3
"""Pluggable credential storage for cosmolex-mcp.

Secrets (API key, username, password, user token) are stored in the operating
system's native secret store via the cross-platform ``keyring`` library:

  * macOS    -> Keychain
  * Windows  -> Credential Manager
  * Linux    -> Secret Service (GNOME Keyring / KWallet)

Storage order (read):  OS keyring  ->  process env  ->  ~/.cosmolex-mcp/.env file
Storage order (write):  OS keyring  (falls back to the 0600 .env file only when
                                     keyring is unavailable or disabled)

The file fallback provides a chmod-0600 ``.env`` path so the server works on
headless hosts with no Secret Service backend, or when a user opts out of
...
SERVICE_NAME = "cosmolex-mcp"
CONFIG_DIR = Path.home() / ".cosmolex-mcp"
ENV_FILE = CONFIG_DIR / ".env"
_USE_KEYRING_FLAG = "COSMOLEX_MCP_USE_KEYRING"
# ---------------------------------------------------------------------------
...
def _keyring_enabled() -> bool:
    ...
    Opt out with ``COSMOLEX_MCP_USE_KEYRING=0`` (or false/no/off). A failing
...
def load_into_environ(keys: list[str]) -> None:
    ...
    Drop-in replacement for direct ``os.environ.get(...)`` reads — values now
    resolve through keyring first (OS keyring -> env -> .env file). Only sets a
    key if not already present in the environment.
    """
    for key in keys:
